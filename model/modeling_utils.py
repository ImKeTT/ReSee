...
# from transformers.models.modeling_bert import (BertConfig, load_tf_weights_in_bert, BERT_PRETRAINED_MODEL_ARCHIVE_MAP)
from transformers.models.gpt2.configuration_gpt2 import GPT2Config
from transformers.models.gpt2.modeling_gpt2 import GPT2PreTrainedModel
from transformers.modeling_utils import PreTrainedModel
from transformers.models.bert.configuration_bert import BertConfig

class MultimodalTrainedModel(PreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs):
        super(MultimodalTrainedModel, self).__init__(config, *inputs, **kwargs)
    ...
    def _decode_step(self, input_ids, past):
        ...
        if self.additional_len == 0:
            next_token_idx = token_len - 1
        else:
            if token_len == 2: ## at step 0: bos + mask token
                assert self._do_output_past(outputs)
                next_token_idx = 1
            else:

                next_token_idx = token_len - self.additional_len - 1
    ...
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            ...
            if cur_len == 1:
                token_len = 2 + self.additional_len
                next_token_idx = 1
            else:
                assert cur_len > 1
                if not self._do_output_past(outputs):
                    token_len = cur_len + 1 + self.additional_len
        ...
        if cur_len == max_length:
            ...
            input_ids[:, -1].masked_fill_(cur_unfinished.to(dtype=torch.uint8),
                                          eos_token_ids[0])  # torch version problem by Jokie
        ...
        while cur_len < max_length:
            ...
            if do_sample:
                ...
                next_words = next_words.view(batch_size,
                                             TOPN_PER_BEAM * num_beams)  # (batch_size, TOPN_PER_BEAM * num_beams)
                ...
                next_scores, next_words = torch.topk(_scores, TOPN_PER_BEAM * num_beams, dim=1, largest=True,
                                                     sorted=True)
