...
from utils.pycocoevalcap.eval import COCOEvalCap, RedditEvalCap, MMDialogEvalCap
...
def evaluate_on_wow_multimodal(eval_file, outfile=None, method='nlg-eval'):
    """
    eval_file: TSV file, each row is [image_key, json format list of captions].
             Each caption is a dict, with fields "caption", "conf".
    label_file: JSON file of ground truth captions in COCO format.
    """
    # ---------------distict calculation
    ...
    if eval_file.endswith('.tsv'):
        ...
        with open(eval_file, 'r') as fp:
            for line in fp:
                line = eval(line)
                cap = skip_comma(line['response_pred']).strip()
                gt_cap = line['response']
                # for evaluate the distict
                cap_for_cal = cap.strip().split()
                results_len.append(len(cap_for_cal))
                unigram_counter.update(cap_for_cal)
                bigram_counter.update(ngrams(cap_for_cal, 2))
        ...
        raise ValueError('unknown prediction result file format: {}'.format(eval_file))

    Evaltool = MMDialogEvalCap(gt, pred, method, 'corpus')

    # evaluate results
    # SPICE will take a few minutes the first time, but speeds up due to caching
    Evaltool.evaluate()
    result = Evaltool.eval

    distinct_1 = len(unigram_counter) / (sum(unigram_counter.values()))
    distinct_2 = len(bigram_counter) / (sum(bigram_counter.values()))
    sent_len = np.mean(results_len)

    result['distinct_1'] = distinct_1
    result['distinct_2'] = distinct_2
    result['avg_len'] = sent_len
    ...
    if not outfile:
        ...
        print(result)
        with open(outfile, 'w') as fp:
            json.dump(result, fp, indent=4, cls=NpEncoder)
    return result

def evaluate_on_wow_multimodal_training(eval_file):
    """
    eval_file: TSV file, each row is [image_key, json format list of captions].
             Each caption is a dict, with fields "caption", "conf".
    label_file: JSON file of ground truth captions in COCO format.
    ...
    if eval_file.endswith('.tsv'):
        ...
        with open(eval_file, 'r') as fp:
            for line in fp:
                line = eval(line)
                cap = skip_comma(line['response_pred'])
                gt_cap = line['response']
                # for evaluate the distict
                cap_for_cal = cap.strip().split()
                results.append(cap)
    ...
    Evaltool = MMDialogEvalCap(gt, pred, 'custom', 'corpus', True)
